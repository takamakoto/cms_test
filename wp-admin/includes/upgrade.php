...
function upgrade_130() {
	...
	$posts = $wpdb->get_results("SELECT ID, post_title, post_email, post_content, post_excerpt, guid, post_date, post_name, post_status, post_author FROM $wpdb->posts");
	if ($posts) {
		foreach ($posts as $post) {
			$post_content = addslashes(deslash($post->post_content));
			$post_title = addslashes(deslash($post->post_title));
			$post_email = addcslashes(deslash($post->post_email));
			...
			$wpdb->update( $wpdb->posts, compact('post_title', 'post_email', 'post_content', 'post_excerpt', 'guid'), array('ID' => $post->ID) );
